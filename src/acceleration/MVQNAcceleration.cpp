...
void MVQNAcceleration::restartIMVJ()
{
  ...
  if (_imvjRestartType == MVQNAcceleration::RS_SVD) {
    ...
    PRECICE_INFO("truncated SVD of Jacobian " << rankAfter << ", new modes: " << rankAfter - rankBefore << ", truncated modes: " << waste << " avg rank: " << _avgRank / _nbRestarts);
    ...
    if (_matrixV_RSLS.cols() > 0) {
      ...
      if (_filter != Acceleration::NOFILTER) {
        ...
        PRECICE_INFO("Number of deleted columns in RS-lS: " << delIndices.size());
      }
